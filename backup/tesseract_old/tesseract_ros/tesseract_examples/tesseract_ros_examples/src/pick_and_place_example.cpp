...
bool PickAndPlaceExample::run()
{
  ...
  if (plotting_)
    plotter->plotTrajectory(pick_prob->GetKin()->getJointNames(),
                            planning_response.trajectory.leftCols(
  ...
  tesseract_->getEnvironment()->setState(planning_response.joint_names,
  ...
  // Plot the resulting trajectory
  if (plotting_)
    plotter->plotTrajectory(planning_response_place.joint_names,
                            planning_response_place.trajectory.leftCols(
