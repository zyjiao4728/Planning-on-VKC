...
bool Tesseract::registerDefaultFwdKinSolvers()
{
  ...
  for (const auto& group : srdf_model_->getGroups())
  {
    ...
    {
      CONSOLE_BRIDGE_logError("[%s]Link groups are currently not supported! group name: %s", 
                              __func__, 
                              group.name_.c_str());
      success = false;
    }
    ...
    {
      CONSOLE_BRIDGE_logError("[%s]Subgroups are currently not supported! subgroup name: %s", 
                              __func__, 
                              group.name_.c_str());
      success = false;
    }
...
bool Tesseract::registerDefaultInvKinSolvers()
{
  ...
  for (const auto& group : srdf_model_->getGroups())
  {
    ...
    if (!group.joints_.empty())
    {
      CONSOLE_BRIDGE_logError("[%s]Joint groups are currently not supported by inverse kinematics! group name: %s",
                              __func__,
                              group.name_.c_str());
      success = false;
    }
    ...
    if (!group.links_.empty())
    {
      CONSOLE_BRIDGE_logError("[%s]Link groups are currently not supported by inverse kinematics! group name: %s",
                              __func__,
                              group.name_.c_str());
      success = false;
    }
    ...
    if (!group.subgroups_.empty())
    {
      CONSOLE_BRIDGE_logError("[%s]Subgroups are currently not supported! group name: %s",
                              __func__,
                              group.name_.c_str());
      success = false;
    }
