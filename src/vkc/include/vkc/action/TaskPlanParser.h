...
private:
    void SplitString(std::vector<std::string> &fields, const std::string &str, const std::string &seperator)
    {
        boost::split(fields, str, boost::is_any_of(seperator), boost::token_compress_on);
    }

    ActionBase::Ptr ParseActionSymbol(const std::string &line)
    {
        auto start_pos = line.find("(") != std::string::npos ? line.find("(") + 1 : 0;
        auto end_pos = line.find(")") != std::string::npos ? line.find(")") : line.length();
        auto substr_len = end_pos - start_pos;
        fprintf(stdout, "[%s]line text: %s\n", __func__, line.substr(start_pos, substr_len).c_str());
