...
class ActionCreatorFactory {
 ...
 private:
  ActionBase::Ptr CreatePickAction(std::vector<std::string> &action_meta) {
    ...
    if (!picked_obj) {
      ROS_ERROR("[%s]invalid picked object: %s", __func__,
                action_meta[2].c_str());
      return nullptr;
    }
  ...
  ActionBase::Ptr CreatePlaceAction(std::vector<std::string> &action_meta) {
    ...
    if (!placed_obj) {
      ROS_ERROR("[%s]invalid placed object: %s", __func__,
                action_meta[2].c_str());
      return nullptr;
    }
  ...
  ActionBase::Ptr CreateUseAction(std::vector<std::string> &action_meta) {
    ...
    if (!used_obj || !applied_obj) {
      if (!used_obj) {
        ROS_ERROR("[%s]invalid use_action used object: %s", __func__,
                  action_meta[2].c_str());
      }
      if (!applied_obj) {
        ROS_ERROR("[%s]invalid use_action action object: %s", __func__,
                  action_meta[3].c_str());
      }
    ...
    return used_obj->CreateUseAction(action_meta[1], *applied_obj);
  }
