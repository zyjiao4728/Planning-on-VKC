...
namespace vkc
{
  ...
  protected:
    ...
    // * @param reach_length how long arm reach is
    // */
    // void setBaseBias(double reach_length, double clearance, double span, double bias);
    // void setBaseBias(BaseBias bb);

    // Eigen::VectorXd genBaseGoal(VKCEnvBasic &env, std::vector<LinkDesiredPose> &link_objectives, BaseBias &bias);

    // Eigen::VectorXd initArmGoal(VKCEnvBasic &env, std::vector<LinkDesiredPose> &link_objectives,
    ...
    // std::vector<double> IKhelper(VKCEnvBasic &env, std::vector<LinkDesiredPose> &link_objectives,
    ...
    // void randBase(Eigen::VectorXd &goal_pose, Eigen::VectorXd &res, BaseBias &bias);
    // TrajOptProb::Ptr moveArm(VKCEnvBasic &env, int n_steps, Eigen::Vector3d p_goal, Eigen::Quaterniond q_goal, Eigen::VectorXd base_pose);
    // Eigen::VectorXd optArmGoal(VKCEnvBasic &env, std::vector<LinkDesiredPose> &link_objectives,
    //                            std::vector<JointDesiredPose> &joint_objectives, Eigen::VectorXd base_pose);
