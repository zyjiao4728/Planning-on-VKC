...
class BaseStick : public BaseObject {
 public:
  ...
  BaseStick(std::string object_name, double length = 1.0)
  ...
  bool createObject() override {
    ...
    stick_link_visual->origin.translation() +=
        Eigen::Vector3d(0, -length_ / 2, 0);
    stick_link_visual->geometry =
        std::make_shared<tesseract_geometry::Box>(0.1, length_, 0.1);
    ...
    stick_link_joint.parent_to_joint_origin_transform =
        Eigen::Isometry3d::Identity();
    stick_link_joint.parent_to_joint_origin_transform.translation() +=
    ...
    // stick_tip_link_visual->geometry =
    // std::make_shared<tesseract_geometry::Box>(0.1, 0.1, 0.1); material_name =
    // stick_tip_link.getName() + "_color"; stick_tip_link_visual->material =
    ...
    // stick_tip_joint.parent_to_joint_origin_transform =
    // Eigen::Isometry3d::Identity();
    // stick_tip_joint.parent_to_joint_origin_transform.translation() +=
    // Eigen::Vector3d(0.0, length_, 0.0);

    // link_map_[stick_tip_link.getName()] =
    // std::make_shared<Link>(std::move(stick_tip_link));
    // joint_map_[stick_tip_joint.getName()] =
    // std::make_shared<Joint>(std::move(stick_tip_joint));

    AttachLocation attach_location("attach_" + stick_link.getName(),
                                   stick_link.getName());
