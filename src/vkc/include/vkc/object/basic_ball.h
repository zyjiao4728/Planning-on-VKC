...
public:
  ...
  BaseBall(std::string object_name, double radius) : BaseObject(object_name), radius_(radius)
  {
  ...
  bool createObject() override
  {
    ...
    ball_link_visual->geometry = std::make_shared<tesseract_geometry::Sphere>(radius_);
    ...
    ball_joint.parent_to_joint_origin_transform.translation() += Eigen::Vector3d(0, 0, radius_ / 2.0);

    link_map_[base_link.getName()] = std::make_shared<Link>(std::move(base_link));
    link_map_[ball_link.getName()] = std::make_shared<Link>(std::move(ball_link));

    joint_map_[ball_joint.getName()] = std::make_shared<Joint>(std::move(ball_joint));

    // Add an attach location
    AttachLocation attach_location("attach_" + ball_link.getName(), ball_link.getName());
    attach_location.local_joint_origin_transform.translation() += Eigen::Vector3d((radius_+0.1)*sqrt(2)/2.0, 0.0,(radius_+0.1)*sqrt(2)/2.0); 
    attach_location.local_joint_origin_transform.linear() = Eigen::Quaterniond(0.0, 0.3827, 0.0, -0.9239).matrix(); 
