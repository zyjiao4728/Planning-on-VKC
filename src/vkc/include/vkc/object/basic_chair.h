...
public:
  ...
  BaseChair(std::string object_name, double h = 0.45, double w = 0.6, double d = 0.6) : BaseObject(object_name), h_(h), w_(w), d_(d)
  {
  ...
  bool createObject() override
  {
    ...
    base_link_visual->origin.translation() += Eigen::Vector3d(0, 0, chair_z + h_ - chair_top_thickness / 2);
    base_link_visual->geometry = std::make_shared<tesseract_geometry::Box>(w_, d_, chair_top_thickness);
    ...
    chair_rest_link_visual->geometry = std::make_shared<tesseract_geometry::Box>(w_, chair_top_thickness, rest_height);
    material_name = chair_rest_link.getName() + "_color";
    chair_rest_link_visual->material = std::make_shared<Material>(material_name);
    ...
    leg1_link_visual->geometry = std::make_shared<tesseract_geometry::Box>(0.055, 0.055, h_ - chair_top_thickness);
    ...
    leg2_link_visual->geometry = std::make_shared<tesseract_geometry::Box>(0.055, 0.055, h_ - chair_top_thickness);
    ...
    leg3_link_visual->geometry = std::make_shared<tesseract_geometry::Box>(0.055, 0.055, h_ - chair_top_thickness);
    ...
    leg4_link_visual->geometry = std::make_shared<tesseract_geometry::Box>(0.055, 0.055, h_ - chair_top_thickness);
    ...
    chair_rest_joint.parent_to_joint_origin_transform = Eigen::Isometry3d::Identity();
    ...
    leg1_joint.parent_to_joint_origin_transform.translation() +=
        Eigen::Vector3d(0.0 + 1 * (w_ / 2 - 0.0375), chair_y + 1 * (d_ / 2 - 0.0375), chair_z + h_ / 2);
    ...
    link_map_[chair_rest_link.getName()] = std::make_shared<Link>(std::move(chair_rest_link));
    link_map_[leg1_link.getName()] = std::make_shared<Link>(std::move(leg1_link));
    link_map_[leg2_link.getName()] = std::make_shared<Link>(std::move(leg2_link));
    link_map_[leg3_link.getName()] = std::make_shared<Link>(std::move(leg3_link));
    link_map_[leg4_link.getName()] = std::make_shared<Link>(std::move(leg4_link));

    joint_map_[chair_rest_joint.getName()] = std::make_shared<Joint>(std::move(chair_rest_joint));
    joint_map_[leg1_joint.getName()] = std::make_shared<Joint>(std::move(leg1_joint));
    joint_map_[leg2_joint.getName()] = std::make_shared<Joint>(std::move(leg2_joint));
    joint_map_[leg3_joint.getName()] = std::make_shared<Joint>(std::move(leg3_joint));
    joint_map_[leg4_joint.getName()] = std::make_shared<Joint>(std::move(leg4_joint));
