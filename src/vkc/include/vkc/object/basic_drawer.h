...
public:
  ...
  bool createObject() override
  {
    ...
    // base_link_visual->geometry = std::make_shared<tesseract_geometry::Box>(0.1, 0.1, 0.1);
    ...
    drawer_visual->geometry = std::make_shared<tesseract_geometry::Box>(drawer_length_, drawer_width_, drawer_height_);
    ...
    drawer_inertial->ixx = 1.0 / 2.0 * drawer_mass_ * (std::pow(drawer_width_, 2) + std::pow(drawer_height_, 2));
    drawer_inertial->iyy = 1.0 / 2.0 * drawer_mass_ * (std::pow(drawer_length_, 2) + std::pow(drawer_height_, 2));
    drawer_inertial->izz = 1.0 / 2.0 * drawer_mass_ * (std::pow(drawer_length_, 2) + std::pow(drawer_width_, 2));
    ...
    left_wall_inertial->ixx = 1.0 / 2.0 * wall_mass_ * (std::pow(wall_thickness_, 2) + std::pow(side_wall_height_, 2));
    left_wall_inertial->iyy =
        1.0 / 2.0 * wall_mass_ * (std::pow(side_wall_length_, 2) + std::pow(side_wall_height_, 2));
    left_wall_inertial->izz = 1.0 / 2.0 * wall_mass_ * (std::pow(side_wall_length_, 2) + std::pow(wall_thickness_, 2));
    ...
    left_wall_visual->geometry =
        std::make_shared<tesseract_geometry::Box>(side_wall_length_, wall_thickness_, side_wall_height_);
    ...
    right_wall_inertial->ixx = 1.0 / 2.0 * wall_mass_ * (std::pow(wall_thickness_, 2) + std::pow(side_wall_height_, 2));
    right_wall_inertial->iyy =
        1.0 / 2.0 * wall_mass_ * (std::pow(side_wall_length_, 2) + std::pow(side_wall_height_, 2));
    right_wall_inertial->izz = 1.0 / 2.0 * wall_mass_ * (std::pow(side_wall_length_, 2) + std::pow(wall_thickness_, 2));
    ...
    bottom_wall_inertial->ixx = 1.0 / 2.0 * wall_mass_ * (std::pow(wall_thickness_, 2) + std::pow(base_wall_width_, 2));
    bottom_wall_inertial->iyy =
        1.0 / 2.0 * wall_mass_ * (std::pow(base_wall_length_, 2) + std::pow(wall_thickness_, 2));
    bottom_wall_inertial->izz =
        1.0 / 2.0 * wall_mass_ * (std::pow(base_wall_length_, 2) + std::pow(base_wall_width_, 2));
    ...
    bottom_wall_visual->geometry =
        std::make_shared<tesseract_geometry::Box>(base_wall_length_, base_wall_width_, wall_thickness_);
    ...
    top_wall_inertial->ixx = 1.0 / 2.0 * wall_mass_ * (std::pow(wall_thickness_, 2) + std::pow(base_wall_width_, 2));
    top_wall_inertial->iyy = 1.0 / 2.0 * wall_mass_ * (std::pow(base_wall_length_, 2) + std::pow(wall_thickness_, 2));
    top_wall_inertial->izz = 1.0 / 2.0 * wall_mass_ * (std::pow(base_wall_length_, 2) + std::pow(base_wall_width_, 2));
    ...
    back_wall_inertial->ixx =
        1.0 / 2.0 * wall_mass_ * (std::pow(back_wall_length_, 2) + std::pow(back_wall_height_, 2));
    back_wall_inertial->iyy = 1.0 / 2.0 * wall_mass_ * (std::pow(back_wall_height_, 2) + std::pow(wall_thickness_, 2));
    back_wall_inertial->izz =
        1.0 / 2.0 * wall_mass_ * (std::pow(back_wall_length_, 2) + std::pow(back_wall_length_, 2));
    ...
    back_wall_visual->geometry =
        std::make_shared<tesseract_geometry::Box>(wall_thickness_, back_wall_length_, back_wall_height_);
    ...
    handle_left_inertial->ixx = 1 / 2 * wall_mass_ * (std::pow(handle_thickness_, 2) + std::pow(handle_thickness_, 2));
    handle_left_inertial->iyy = 1 / 2 * wall_mass_ * (std::pow(handle_thickness_, 2) + std::pow(handle_gap_, 2));
    handle_left_inertial->izz = 1 / 2 * wall_mass_ * (std::pow(handle_thickness_, 2) + std::pow(handle_gap_, 2));
    ...
    handle_left_visual->geometry =
        std::make_shared<tesseract_geometry::Box>(handle_gap_, handle_thickness_, handle_thickness_);
    ...
    handle_right_inertial->ixx = 1 / 2 * wall_mass_ * (std::pow(handle_thickness_, 2) + std::pow(handle_thickness_, 2));
    handle_right_inertial->iyy = 1 / 2 * wall_mass_ * (std::pow(handle_thickness_, 2) + std::pow(handle_gap_, 2));
    handle_right_inertial->izz = 1 / 2 * wall_mass_ * (std::pow(handle_thickness_, 2) + std::pow(handle_gap_, 2));
    ...
    handle_link_inertial->ixx =
        1 / 2 * wall_mass_ * (std::pow(handle_thickness_, 2) + std::pow(handle_span_ + 2 * handle_thickness_, 2));
    handle_link_inertial->iyy = 1 / 2 * wall_mass_ * (std::pow(handle_thickness_, 2) + std::pow(handle_thickness_, 2));
    ...
    handle_link_visual->geometry = std::make_shared<tesseract_geometry::Box>(
        handle_thickness_, handle_span_ + 2 * handle_thickness_, handle_thickness_);
    ...
    handle_top_cover_inertial->ixx =
        1 / 2 * wall_mass_ * (std::pow(handle_thickness_, 2) + std::pow(handle_span_ + 2 * handle_thickness_, 2));
    handle_top_cover_inertial->iyy = 1 / 2 * wall_mass_ * (std::pow(handle_gap_, 2) + std::pow(handle_thickness_, 2));
    handle_top_cover_inertial->izz =
        1 / 2 * wall_mass_ * (std::pow(handle_gap_, 2) + std::pow(handle_span_ + 2 * handle_thickness_, 2));
    handle_top_cover.inertial = handle_top_cover_inertial;

    Visual::Ptr handle_top_cover_visual = std::make_shared<Visual>();
    handle_top_cover_visual->origin = Eigen::Isometry3d::Identity();
    handle_top_cover_visual->geometry = std::make_shared<tesseract_geometry::Box>(
        handle_gap_ + handle_thickness_, handle_span_ + 2 * handle_thickness_, handle_thickness_ * 0.3);
    material_name = handle_top_cover.getName() + "_color";
    handle_top_cover_visual->material = std::make_shared<Material>(material_name);
    ...
    handle_bottom_cover_inertial->iyy =
        1 / 2 * wall_mass_ * (std::pow(handle_gap_, 2) + std::pow(handle_thickness_, 2));
    handle_bottom_cover_inertial->izz =
        1 / 2 * wall_mass_ * (std::pow(handle_gap_, 2) + std::pow(handle_span_ + 2 * handle_thickness_, 2));
    handle_bottom_cover.inertial = handle_bottom_cover_inertial;

    Visual::Ptr handle_bottom_cover_visual = std::make_shared<Visual>();
    handle_bottom_cover_visual->origin = Eigen::Isometry3d::Identity();
    handle_bottom_cover_visual->geometry = std::make_shared<tesseract_geometry::Box>(
        handle_thickness_*0.3, handle_span_ + 2 * handle_thickness_, (handle_gap_ + handle_thickness_)*1.4142);
    material_name = handle_bottom_cover.getName() + "_color";
    handle_bottom_cover_visual->material = std::make_shared<Material>(material_name);
    ...
    Collision::Ptr handle_bottom_cover_collision = std::make_shared<Collision>();
    handle_bottom_cover_collision->origin = handle_bottom_cover_visual->origin;
    handle_bottom_cover_collision->geometry = handle_bottom_cover_visual->geometry;
    ...
    */
    ...
    base_drawer_joint.parent_to_joint_origin_transform = Eigen::Isometry3d::Identity();
    ...
    base_left_wall_joint.parent_to_joint_origin_transform = Eigen::Isometry3d::Identity();
    base_left_wall_joint.parent_to_joint_origin_transform.translation() +=
        Eigen::Vector3d(-gap_, -(drawer_width_ / 2 + wall_thickness_ / 2) - gap_, 0);
    ...
    base_right_wall_joint.parent_to_joint_origin_transform = Eigen::Isometry3d::Identity();
    ...
    base_bottom_wall_joint.parent_to_joint_origin_transform = Eigen::Isometry3d::Identity();
    base_bottom_wall_joint.parent_to_joint_origin_transform.translation() +=
        Eigen::Vector3d(-gap_, 0, -(drawer_height_ / 2 + wall_thickness_ / 2) - gap_);
    ...
    base_top_wall_joint.parent_to_joint_origin_transform = Eigen::Isometry3d::Identity();
    ...
    base_back_wall_joint.parent_to_joint_origin_transform = Eigen::Isometry3d::Identity();
    base_back_wall_joint.parent_to_joint_origin_transform.translation() +=
        Eigen::Vector3d(-(drawer_length_ / 2 + gap_ + wall_thickness_ / 2), 0, 0);
    ...
    drawer_handle_left_joint.parent_to_joint_origin_transform = Eigen::Isometry3d::Identity();
    drawer_handle_left_joint.parent_to_joint_origin_transform.translation() +=
        Eigen::Vector3d(drawer_length_ / 2 + handle_gap_ / 2, -(handle_span_ / 2 + handle_thickness_ / 2), 0);
    drawer_handle_left_joint.type = JointType::FIXED;

    Joint drawer_handle_right_joint(object_name_ + "_drawer_handle_right_joint");
    drawer_handle_right_joint.parent_link_name = drawer.getName();
    drawer_handle_right_joint.child_link_name = handle_right.getName();
    drawer_handle_right_joint.parent_to_joint_origin_transform = Eigen::Isometry3d::Identity();
    ...
    drawer_handle_link_joint.parent_to_joint_origin_transform = Eigen::Isometry3d::Identity();
    drawer_handle_link_joint.parent_to_joint_origin_transform.translation() +=
        Eigen::Vector3d(drawer_length_ / 2 + handle_gap_ + handle_thickness_ / 2, 0, 0);
    drawer_handle_link_joint.type = JointType::FIXED;

    /*
    Joint drawer_handle_top_cover_joint(object_name_ + "_drawer_handle_top_cover_joint");
    ...
    drawer_handle_top_cover_joint.parent_to_joint_origin_transform = Eigen::Isometry3d::Identity();
    drawer_handle_top_cover_joint.parent_to_joint_origin_transform.translation() +=
        Eigen::Vector3d(drawer_length_ / 2 + handle_gap_ /2 + handle_thickness_ / 2, 0,
                        -handle_thickness_ * 0.3 / 2.0 - 2.0 * handle_thickness_);
    drawer_handle_top_cover_joint.type = JointType::FIXED;

    Joint drawer_handle_bottom_cover_joint(object_name_ + "_drawer_handle_bottom_cover_joint");
    drawer_handle_bottom_cover_joint.parent_link_name = handle_top_cover.getName();
    drawer_handle_bottom_cover_joint.child_link_name = handle_bottom_cover.getName();
    drawer_handle_bottom_cover_joint.parent_to_joint_origin_transform = Eigen::Isometry3d::Identity();
    drawer_handle_bottom_cover_joint.parent_to_joint_origin_transform.translation() +=
        Eigen::Vector3d(-handle_thickness_ * 0.1 / 2, 0, -(handle_gap_ + handle_thickness_) / 2);
    drawer_handle_bottom_cover_joint.parent_to_joint_origin_transform.linear() =
        Eigen::AngleAxisd(0.78539816339, Eigen::Vector3d::UnitY()).toRotationMatrix();
    ...
    */
    ...
    link_map_[left_wall.getName()] = std::make_shared<Link>(std::move(left_wall));
    link_map_[right_wall.getName()] = std::make_shared<Link>(std::move(right_wall));
    link_map_[bottom_wall.getName()] = std::make_shared<Link>(std::move(bottom_wall));
    link_map_[top_wall.getName()] = std::make_shared<Link>(std::move(top_wall));
    link_map_[back_wall.getName()] = std::make_shared<Link>(std::move(back_wall));
    link_map_[handle_link.getName()] = std::make_shared<Link>(std::move(handle_link));
    link_map_[handle_left.getName()] = std::make_shared<Link>(std::move(handle_left));
    link_map_[handle_right.getName()] = std::make_shared<Link>(std::move(handle_right));
    // link_map_[handle_top_cover.getName()] = std::make_shared<Link>(std::move(handle_top_cover));
    // link_map_[handle_bottom_cover.getName()] = std::make_shared<Link>(std::move(handle_bottom_cover));

    joint_map_[base_drawer_joint.getName()] = std::make_shared<Joint>(std::move(base_drawer_joint));
    joint_map_[base_left_wall_joint.getName()] = std::make_shared<Joint>(std::move(base_left_wall_joint));
    joint_map_[base_right_wall_joint.getName()] = std::make_shared<Joint>(std::move(base_right_wall_joint));
    joint_map_[base_back_wall_joint.getName()] = std::make_shared<Joint>(std::move(base_back_wall_joint));
    joint_map_[base_bottom_wall_joint.getName()] = std::make_shared<Joint>(std::move(base_bottom_wall_joint));
    joint_map_[base_top_wall_joint.getName()] = std::make_shared<Joint>(std::move(base_top_wall_joint));
    // joint_map_[drawer_handle_bottom_cover_joint.getName()] =
    //     std::make_shared<Joint>(std::move(drawer_handle_bottom_cover_joint));
    joint_map_[drawer_handle_link_joint.getName()] = std::make_shared<Joint>(std::move(drawer_handle_link_joint));
    joint_map_[drawer_handle_left_joint.getName()] = std::make_shared<Joint>(std::move(drawer_handle_left_joint));
    joint_map_[drawer_handle_right_joint.getName()] = std::make_shared<Joint>(std::move(drawer_handle_right_joint));
    ...
    attach_location.local_joint_origin_transform.translation() += Eigen::Vector3d(0.1, 0.0, 0.0);
    attach_location.local_joint_origin_transform.linear() = Eigen::Quaterniond(0.0, 0.0, 0.7071, 0.7071).matrix();
...
private:
  ...
  double side_wall_height_, side_wall_length_, base_wall_width_, base_wall_length_, back_wall_height_,
      back_wall_length_, handle_gap_, handle_thickness_, handle_span_;
};
}  // namespace vkc
