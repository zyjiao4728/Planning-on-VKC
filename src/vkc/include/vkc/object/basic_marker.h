...
public:
  ...
  BaseMarker(std::string object_name, double radius) : BaseObject(object_name), radius_(radius)
  {
  ...
  bool createObject() override
  {
    ...
    marker_link_visual->geometry = std::make_shared<tesseract_geometry::Box>(radius_, radius_, radius_);
    ...
    // marker_link_collision->geometry = std::make_shared<tesseract_geometry::Sphere>(radius_);
    ...
    marker_joint.parent_to_joint_origin_transform = Eigen::Isometry3d::Identity();
    marker_joint.parent_to_joint_origin_transform.translation() += Eigen::Vector3d(0, 0, 0);

    link_map_[base_link.getName()] = std::make_shared<Link>(std::move(base_link));
    link_map_[marker_link.getName()] = std::make_shared<Link>(std::move(marker_link));

    joint_map_[marker_joint.getName()] = std::make_shared<Joint>(std::move(marker_joint));

    // Add an attach location
    // AttachLocation attach_location("attach_" + marker_link.getName(), marker_link.getName());
    // attach_location.local_joint_origin_transform.translation() += Eigen::Vector3d(0, 0, 0);
    // attach_location.local_joint_origin_transform.linear() = Eigen::Quaterniond(1, 0, 0, 0).matrix();
    // attach_location.fixed_base = true;
    AttachLocation attach_location("attach_" + marker_link.getName(), marker_link.getName());
    attach_location.local_joint_origin_transform.translation() += Eigen::Vector3d(0, 0, 0.2);
    attach_location.local_joint_origin_transform.linear() = Eigen::Quaterniond(0.7071,0,0.7071,0).matrix();
    // attach_location.local_joint_origin_transform.linear() = Eigen::Quaterniond(1, 0, 0, 0).matrix();
