...
class BaseHook : public BaseObject {
 public:
  ...
  bool createObject() override {
    ...
    hook_handle_link_visual->geometry =
        std::make_shared<tesseract_geometry::Box>(0.1, 0.9, 0.1);
    material_name = hook_handle_link.getName() + "_color";
    hook_handle_link_visual->material =
    ...
    hook_handle_joint.parent_to_joint_origin_transform =
        Eigen::Isometry3d::Identity();
    hook_handle_joint.parent_to_joint_origin_transform.translation() +=
        Eigen::Vector3d(0, 0, 0.05);

    link_map_[hook_handle_link.getName()] =
        std::make_shared<Link>(std::move(hook_handle_link));
    joint_map_[hook_handle_joint.getName()] =
        std::make_shared<Joint>(std::move(hook_handle_joint));
    ...
    hook_tip_link_visual->geometry =
        std::make_shared<tesseract_geometry::Box>(0.3, 0.1, 0.1);
    ...
    hook_tip_joint.parent_to_joint_origin_transform =
        Eigen::Isometry3d::Identity();
    hook_tip_joint.parent_to_joint_origin_transform.translation() +=
        Eigen::Vector3d(0.15, 0.4, 0.0);

    link_map_[hook_tip_link.getName()] =
        std::make_shared<Link>(std::move(hook_tip_link));
    joint_map_[hook_tip_joint.getName()] =
        std::make_shared<Joint>(std::move(hook_tip_joint));

    AttachLocation attach_location("attach_" + hook_handle_link.getName(),
                                   hook_handle_link.getName());
