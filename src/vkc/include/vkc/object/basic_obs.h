...
public:
  ...
  BaseObs(std::string object_name, double r, double h) : BaseObject(object_name), r_(r), h_(h)
  {
  ...
  bool createObject() override
  {
    ...
    obs_link_visual->geometry = std::make_shared<tesseract_geometry::Cylinder>(r_, h_);
    ...
    obs_joint.parent_to_joint_origin_transform.translation() += Eigen::Vector3d(0, 0, h_ / 2.0);
    ...
    joint_map_[obs_joint.getName()] = std::make_shared<Joint>(std::move(obs_joint));
