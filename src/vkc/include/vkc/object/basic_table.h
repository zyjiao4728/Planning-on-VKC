#ifndef VKC_BASIC_TABLE_H
#define VKC_BASIC_TABLE_H

#include <vkc/object/basic_object.h>

namespace vkc
{
class BaseTable : public BaseObject
{
public:
  using Ptr = std::shared_ptr<BaseTable>;
  using ConstPtr = std::shared_ptr<const BaseTable>;

  BaseTable(std::string object_name, double h=0.8, double w=1.5, double d=0.9) : BaseObject(object_name), h_(h), w_(w), d_(d)
  {
    color_ = Eigen::Vector4d(153.0 / 255.0, 51.0 / 255.0, 0, 1);
  }
  ...
  bool createObject() override
  {
    ...
    base_link_visual->origin.translation() += Eigen::Vector3d(0, 0, table_z + h_ - table_top_thickness / 2);
    base_link_visual->geometry = std::make_shared<tesseract_geometry::Box>(w_, d_, table_top_thickness);
    ...
    leg1_link_visual->geometry = std::make_shared<tesseract_geometry::Box>(0.1, 0.1, h_ - table_top_thickness);
    ...
    leg2_link_visual->geometry = std::make_shared<tesseract_geometry::Box>(0.1, 0.1, h_ - table_top_thickness);
    ...
    leg3_link_visual->geometry = std::make_shared<tesseract_geometry::Box>(0.1, 0.1, h_ - table_top_thickness);
    ...
    leg4_link_visual->geometry = std::make_shared<tesseract_geometry::Box>(0.1, 0.1, h_ - table_top_thickness);
    ...
    leg1_joint.parent_to_joint_origin_transform.translation() +=
        Eigen::Vector3d(0.0 + 1 * (w_ / 2 - 0.05), table_y + 1 * (d_ / 2 - 0.05), table_z + h_ / 2);
...
private:
  double table_top_thickness = 0.04;
