...
class BaseTable : public BaseObject {
 public:
  ...
  BaseTable(std::string object_name, double h = 0.8, double w = 1.5,
            double d = 0.9)
  ...
  bool createObject() override {
    ...
    base_link_visual->origin.translation() +=
        Eigen::Vector3d(0, 0, table_z + h_ - table_top_thickness / 2);
    base_link_visual->geometry =
        std::make_shared<tesseract_geometry::Box>(w_, d_, table_top_thickness);
    ...
    leg1_link_visual->geometry = std::make_shared<tesseract_geometry::Box>(
        0.1, 0.1, h_ - table_top_thickness);
    ...
    leg1_joint.parent_to_joint_origin_transform.translation() +=
        Eigen::Vector3d(0.0 + 1 * (w_ / 2 - 0.05),
