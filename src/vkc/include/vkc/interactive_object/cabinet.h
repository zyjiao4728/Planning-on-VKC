...
class Cabinet : public IObject {
 public:
  ...
  Cabinet(const std::string& name) : name_(name) {
  ...
  double OpenPose() { return joint_poses_["door_opened"]; }

  double ClosePose() { return joint_poses_["door_closed"]; }

  std::string AttachLinkName() { return attach_handle_name_; }
      ...
      const std::string& pose_name)  // name of the pose where to place the ball
  {
    ...
    joint_objectives.emplace_back(joint_name_, joint_poses_[pose_name]);
    auto open_door = std::make_shared<PlaceAction>(
        manipulator, attach_handle_name_, link_objectives, joint_objectives);
  ...
  PlaceAction::Ptr CreateOpenDoorAction(const std::string& manipulator) {
    std::vector<LinkDesiredPose> link_objectives;
    std::vector<JointDesiredPose> joint_objectives;
    joint_objectives.emplace_back(
        joint_name_,
        joint_poses_["door_open"]);  // TODO: this may be danager if the poses
                                     // mantained by user

    return std::make_shared<PlaceAction>(manipulator, attach_handle_name_,
  ...
  PlaceAction::Ptr CreateCloseDoorAction(const std::string& manipulator) {
    ...
    joint_objectives.emplace_back(
        joint_name_,
        joint_poses_["door_close"]);  // TODO: this may be danager if the poses
                                      // mantained by user
