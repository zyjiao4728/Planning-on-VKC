...
public:
    ...
    {
        return poses_["current_pose"];
    }

    double* InitPose()
    {
        return poses_["init_pose"].data();
    }
    double* Pose()
    {
        return poses_["fetch_pose"].data();
    }
    double* TargetPose()
    {
        return poses_["target_pose"].data();
    }
                                       ...
                                       const std::string &pose_name)   // name of the pose where to place the ball
    {
        ...
        Eigen::Isometry3d tf;
        tf.translation() = Eigen::Vector3d(pose[0],
                                           pose[1],
                                           pose[2]);
        tf.linear() = Eigen::Quaterniond(pose[3],
                                         pose[4],
                                         pose[5],
                                         pose[6])
                          .matrix();
    ...
    {
        Eigen::Isometry3d tf;
        tf.translation() = Eigen::Vector3d(CurrentPose()[0], CurrentPose()[1], CurrentPose()[2]);
        tf.linear() = Eigen::Quaterniond(CurrentPose()[3], CurrentPose()[4], CurrentPose()[5], CurrentPose()[6]).matrix();

        return std::make_shared<UseAction>(manipulator,
                                           attach_link_name_,
                                           tf,
                                           end_effect_link);   
    }
