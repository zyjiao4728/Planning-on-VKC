...
class Stick : public IUsableObject {
 public:
  ...
  Stick(const std::string& name) : name_(name) {
  ...
  double* Pose() { return pos_; }

  const std::string& AttachLinkName() { return attach_link_name_; }
  ...
  {
    return applied_object.CreateUseAction(manipulator, link_name_);
  }
  ...
  virtual PlaceAction::Ptr CreatePlaceAction(
      const std::string& manipulator, const std::string& pose_name = "") {
    Eigen::Isometry3d tf;

    tf.translation() = Eigen::Vector3d(pos_[0], pos_[1], pos_[2]);
    tf.linear() =
        Eigen::Quaterniond(pos_[3], pos_[4], pos_[5], pos_[6]).matrix();
