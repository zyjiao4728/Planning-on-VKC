...
namespace ur_kinematics {
  ...
  int inverse(Eigen::Isometry3d & trans, std::vector<std::vector<double>> & q_sols_std, double q6_des){

    Eigen::Matrix<double, 4, 4, Eigen::RowMajor> Arowmajor = trans.matrix();
    double * T = Arowmajor.data();
    double* q_sols;

    int sol_num = inverse( T, q_sols, q6_des);

    q_sols_std.clear();

    int sol_id = 0;
    while (sol_id < sol_num){
      std::vector<double> temp_sol = {q_sols[sol_id*6+0],  q_sols[sol_id*6+1],q_sols[sol_id*6+2],q_sols[sol_id*6+3],q_sols[sol_id*6+4],q_sols[sol_id*6+5]};
      q_sols_std.push_back(temp_sol);
    }
...
IKFAST_API bool ComputeIk(const IkReal* eetrans, const IkReal* eerot, const IkReal* pfree, IkSolutionListBase<IkReal>& solutions) {
  ...
  for (size_t i=0; i < num_sols; ++i){
    ...
    for (size_t j=0; j < n; ++j) vinfos[j].foffset = q_sols[i*n+j];
...
IKFAST_API size_t GetNumJoints() { return 6; }
