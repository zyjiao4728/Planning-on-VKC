...
namespace vkc
{
  ...
  PlannerRequest ProbGenerator::genRequest(VKCEnvBasic &env, ActionBase::Ptr action, int n_steps, int n_iter)
  {
  ...
  ProfileDictionary::Ptr ProbGenerator::genCartProfiles_(VKCEnvBasic &env, double collision_margin, int collision_coeff, Eigen::Vector3d pos_coeff, Eigen::Vector3d rot_coeff)
  {
    ...
    profiles->addProfile<TrajOptCompositeProfile>(profile_ns::TRAJOPT_DEFAULT_NAMESPACE, "DEFAULT", trajopt_composite_profile);
    profiles->addProfile<TrajOptPlanProfile>(profile_ns::TRAJOPT_DEFAULT_NAMESPACE, "CARTESIAN", trajopt_plan_profile);

    return profiles;
  }

  PlannerRequest ProbGenerator::genPickProb(VKCEnvBasic &env, PickAction::Ptr act, int n_steps, int n_iter)
  {
    ...
    auto profiles = genCartProfiles_(env, collision_margin, collision_coeff, pos_coeff, rot_coeff);
    setSolverProfile(profiles, n_iter);

    CompositeInstruction program("FREESPACE", CompositeInstructionOrder::ORDERED, manip);

    // set initial pose
    setStartInstruction(program, env_->getActiveJointNames(), env_->getCurrentJointValues());

    // set target pose
    BaseObject::AttachLocation::Ptr attach_location_ptr = env.getAttachLocation(act->getAttachedObject());
    Eigen::Isometry3d pick_pose_world_transform = env.getVKCEnv()->getTesseract()->getLinkTransform(attach_location_ptr->link_name_) *
    ...
    CompositeInstruction seed = generateSeed(program, cur_state, env.getVKCEnv()->getTesseract());
  ...
  trajopt::ProblemConstructionInfo ProbGenerator::genPickProb_test(VKCEnvBasic &env, PickAction::Ptr act, int n_steps)
  {
    ...
    // PickPose->timestep = pci.basic_info.n_steps - 1;
    // PickPose->target_frame = "world";
    // PickPose->target_frame_offset = attach_location_ptr->world_joint_origin_transform;
  ...
  PlannerRequest ProbGenerator::genPlaceProb(VKCEnvBasic &env, PlaceAction::Ptr act, int n_steps, int n_iter)
  {
    ...
    act->addLinkObjectives(LinkDesiredPose(detach_location_ptr->base_link_, detach_pose));
    ...
    //     BasePose->target_frame_offset = env.getVKCEnv()->getTesseract()->getLinkTransform(detach_location_ptr->base_link_);
  ...
  PlannerRequest ProbGenerator::genGotoProb(VKCEnvBasic &env, GotoAction::Ptr act, int n_steps)
  {
    ProblemConstructionInfo pci(std::move(env.getVKCEnv()->getTesseract()->clone()));
  ...
  PlannerRequest ProbGenerator::genUseProb(VKCEnvBasic &env, UseAction::Ptr act, int n_steps)
  {
    ProblemConstructionInfo pci(std::move(env.getVKCEnv()->getTesseract()->clone()));
  ...
  bool ProbGenerator::validateGroupID(Environment::Ptr tesseract, const std::string &group_id)
  {
  ...
  void ProbGenerator::setJointPlanProfile(TrajOptDefaultPlanProfile::Ptr profile, Eigen::VectorXd joint_coeff)
  {
    profile->joint_coeff = joint_coeff;
  }

  void ProbGenerator::setCartPlanProfile(TrajOptDefaultPlanProfile::Ptr profile, Eigen::Vector3d pos_coeff,
  ...
  void ProbGenerator::setSolverProfile(ProfileDictionary::Ptr profiles, int n_iter)
  {
    ...
    profiles->addProfile<TrajOptSolverProfile>(profile_ns::TRAJOPT_DEFAULT_NAMESPACE, "FREESPACE", trajopt_solver_profile);

    return;
  }

  void ProbGenerator::setStartInstruction(CompositeInstruction &program, ros::V_string joint_names, Eigen::VectorXd joint_values)
  {
  ...
  void ProbGenerator::addCollisionTerm(ProblemConstructionInfo &pci, double margin, double coeff)
  {
    ...
    collision->evaluator_type = trajopt::CollisionEvaluatorType::DISCRETE_CONTINUOUS;
    collision->first_step = 0;
    collision->last_step = pci.basic_info.n_steps - 2;
    // collision->gap = 1;
    collision->info = util::createSafetyMarginDataVector(pci.basic_info.n_steps, margin, coeff);
