...
namespace vkc
{
  ...
  TrajOptProb::Ptr ProbGenerator::genPlaceProb(VKCEnvBasic &env, PlaceAction::Ptr act, int n_steps)
  {
    ...
    if (detach_location_ptr->fixed_base)
    {
      ...
      for (int i = 0; i < pci.basic_info.n_steps; ++i)
      {
        ...
        BasePose->target_frame_offset = env.getVKCEnv()->getTesseractEnvironment().getLinkTransform(detach_location_ptr->base_link_);
  ...
  bool ProbGenerator::validateGroupID(EnvironmentMonitor::Ptr tesseract, const std::string &group_id)
  {
    bool isfound_group = false;

    for (const auto &name_ : tesseract->getEnvironment().getGroupNames())
    {
      if (name_ == group_id)
      {
        isfound_group = true;
