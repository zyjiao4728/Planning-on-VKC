...
void run(VKCEnvBasic &env, ActionSeq &actions, int n_steps, int n_iter,
  ...
  for (auto &action : actions) {
    ...
    if (ActionType::PlaceAction != action->getActionType() ||
        std::dynamic_pointer_cast<PlaceAction>(action)->isRigidObject()) {
    ...
    while (!converged && tries++ < 5) {
      TrajOptProb::Ptr prob_ptr =
          prob_generator.genRequest(env, action, n_steps);

      // 6: is half size of the map
      bool on_map_border =
          (abs(abs(prob_ptr->GetInitTraj().bottomRows(1)(0)) - 6) < 1e-6 ||
           abs(abs(prob_ptr->GetInitTraj().bottomRows(1)(1)) - 6) < 1e-6);
      if (on_map_border &&
          string::npos == env.getEndEffectorLink().find("cabinet")) {
        // std::cout << prob_ptr->GetInitTraj().bottomRows(1)(0) << " " <<
        // prob_ptr->GetInitTraj().bottomRows(1)(1) << std::endl;
        if (tries < 4) {
    ...
    env.updateEnv(response.joint_names,
                  response.trajectory.bottomRows(1).transpose(), action);
    plotter->clear();
  }
}
