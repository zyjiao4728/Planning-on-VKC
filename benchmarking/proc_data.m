...
sum(open_door_push_vkc(:,4)>0 & open_door_push_vkc(:,7)>0) / N * 100
open_door_push_ompl = csvread("open_door_pull_ompl.csv");
sum(open_door_push_ompl(:,4)>0 & open_door_push_ompl(:,7)>0) / N * 100
...
sum(open_drawer_pull_vkc(:,4)>0 & open_drawer_pull_vkc(:,7)>0) / N * 100
open_drawer_pull_ompl = csvread("open_drawer_pull_ompl.csv");
sum(open_drawer_pull_ompl(:,4)>0 & open_drawer_pull_ompl(:,7)>0) / N * 100
